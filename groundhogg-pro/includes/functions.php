 ...
 * @param $current Step
 */
function handle_skips( $next, $current ) {

	if ( ! $current->type_is( 'skip' ) ) {
		return $next;
	}
...
add_filter( 'groundhogg/step/next_action', __NAMESPACE__ . '\handle_skips', 10, 2 );
 ...
 * @param $current Step
 */
function handle_loops( $next, $current ) {

	if ( ! $current->type_is( 'loop' ) ) {
		return $next;
	}
	...
	$limit = absint( $current->get_meta( 'limit' ) );
...
add_filter( 'groundhogg/step/next_action', __NAMESPACE__ . '\handle_loops', 10, 2 );
